...
namespace SqlServerLocalization.Modules
{
    ...
    {
        private readonly ConnectionStringSettings _conString;
        private readonly DbProviderFactory _factory;
        private IDbConnection _connection;

        public ConnectionScope()
        {
            var connectionStringName = "DemoDb";
            ...
            if (_conString == null)
                throw new InvalidOperationException("Failed to find connection string named " + connectionStringName +
                                                    " in your configuration file (should exist in the <connectionStrings> element)");

            _factory = DbProviderFactories.GetFactory(_conString.ProviderName);
            if (_factory == null)
                throw new InvalidOperationException("Failed to find provider " + _conString.ProviderName +
                                                    " which is used in connection string " + connectionStringName);
        }
        ...
        public IDbConnection Connection
        {
            get
            {
                if (_connection == null)
                {
                    ...
                    if (_connection == null)
                        throw new InvalidOperationException("Factory " + _conString.ProviderName +
                                                            " failed to create a new connection object.");
